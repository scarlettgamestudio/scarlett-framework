...
export default class GridExt {
  ...
  render(delta) {
    ...
    if (this.enabled) {
      ...
      let offsetX =
        this._game.getActiveCamera().x -
        this._game.getActiveCamera().x % gridSize;
      let offsetY =
        this._game.getActiveCamera().y -
        this._game.getActiveCamera().y % gridSize;
      let zoomDifX = zoom * screenResolution.width * 2.0;
      let zoomDifY = zoom * screenResolution.height * 2.0;
      let howManyX = Math.floor(
        (screenResolution.width + zoomDifX) / gridSize + 2
      );
      let howManyY = Math.floor(
        (screenResolution.height + zoomDifY) / gridSize + 2
      );
      ...
      for (let x = 0; x < howManyX; x++) {
        ...
        if (
          (x * gridSize + offsetX + (alignedX ? gridSize : 0)) %
          upperGridSize
        ) {
          color = dynColor;
        }
      ...
      for (let y = 0; y < howManyY; y++) {
        color = this._gridColor;
        if (
          (y * gridSize + offsetY + (alignedY ? gridSize : 0)) %
          upperGridSize
        ) {
          color = dynColor;
        }
      ...
      if (this._originLines) {
        ...
        this._primitiveRender.drawRectangle(
          new Rectangle(
            -2,
            top - this._gridSize + offsetY,
            4,
            screenResolution.height + zoomDifY
          ),
          this._gridColor
        );
        ...
        this._primitiveRender.drawRectangle(
          new Rectangle(
            left - this._gridSize + offsetX,
            -2,
            screenResolution.width + zoomDifX,
            4
          ),
          this._gridColor
        );
